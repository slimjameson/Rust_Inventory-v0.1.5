...
function PANEL:CreateInventoryGrid()
    ...
    local inventoryX = (screenW - inventoryWidth) / 2
    local inventoryY = screenH - 90 - inventoryHeight - 10
    ...
    for row = 1, rows do
        ...
        for col = 1, cols do
            ...
            slotPanel.OnCursorEntered = function(s)
                s.IsHovered = true
                local slotIndex = (row - 1) * cols + col
                s.HoveredItem = RustInventory.Items and RustInventory.Items.GetInventoryItem(slotIndex) or nil
            end
            ...
            slotPanel.OnMousePressed = function(s, keyCode)
                if keyCode == MOUSE_LEFT then
                    ...
                    elseif RustInventory.Items and RustInventory.Items.HasInventoryItem(slotIndex) then
            ...
            if hasItem and item then
                if item.model and util.IsValidModel(item.model) then
                    if not IsValid(s.ModelPanel) then
                        ...
                        s.ModelPanel:SetMouseInputEnabled(false)
                        s.ModelPanel:SetZPos(-1)
                        s.ModelPanel.LayoutEntity = function(_, ent)
                    ...
                    local iconMat = item.icon and Material(item.icon)
