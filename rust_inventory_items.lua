...
local function GetItemIcon(weapon)
    ...
    return modelMap[class] or nil -- Return nil for unknown weapons to trigger text fallback
end
...
local function CreateItemData(weapon, slot)
    return {
        ...
        icon = GetItemIcon(weapon), -- Optional fallback
        model = RustInventory.Config.ModelMap[weapon:GetClass()] or weapon:GetModel(), -- ADDED THIS LINE
